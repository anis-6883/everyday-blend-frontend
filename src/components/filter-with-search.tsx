...
import { Title, Input, Checkbox, ActionIcon, CheckboxGroup, AdvancedRadio, Tooltip, Button } from "rizzui";
...
export default function FilterWithSearch({
  ...
  const [values, setValues] = useState<string[]>(state[name]?.length ? state[name].split(",") : []);
        ...
        <div className='flex flex-col pt-5'>
          ...
          {filteredData!.length > 5 ? (
            <CollapsibleFilterOptions data={filteredData}>
              <CheckboxGroup values={values} setValues={setValues} className='space-y-3.5 pt-3.5'>
                {filteredData?.slice(5, filteredData.length).map((item: any) => (
                  <Checkbox
                    key={`${item.name}-key-${item.id}`}
                    label={
                      <FilterOption
          ...
          {!filteredData.length ? <div className='text-gray-500'>No result found</div> : null}
        </div>
      </div>
    </>
  );
...
function FilterOptionSearch({
  ...
  return (
    <div className={cn("absolute end-0 z-10 flex w-full", isSearchOpen ? "-top-1" : "-top-0.5")}>
      {isSearchOpen && (
        <Input
          type='search'
          size='sm'
      ...
      <ActionIcon
        ...
        rounded='full'
        className={cn("absolute z-10 transition-none", isSearchOpen ? "end-1.5 top-0.5" : "end-0")}
        onClick={() => {
          setSearchOpen(!isSearchOpen);
          setSearchTerm("");
        }}
      >
        {isSearchOpen ? <PiXBold className='h-auto w-4' /> : <PiMagnifyingGlassBold className='h-auto' />}
      </ActionIcon>
    </div>
  );
}
  ...
  return (
    <div className='flex items-center justify-between'>
      <div className='relative flex shrink-0 items-center'>
        {color ? <span className='me-1.5 block h-4 w-4 rounded-full' style={{ backgroundColor: color }} /> : null}
      ...
      {String(count) ? <span className='text-xs opacity-80'>{count}</span> : null}
    </div>
  );
}

function CollapsibleFilterOptions({ data, children }: React.PropsWithChildren<{ data: FilterOptions[] }>) {
  return (
    <div className='w-full'>
      <Disclosure>
        {({ open }) => (
          <>
            <Disclosure.Panel className='space-y-3.5'>{children}</Disclosure.Panel>
